<div class="aq-cartmini-area aq-cartmini-active d-flex flex-column justify-content-between">
    ...
    <div class="aq-cartmini-body">
        @php
            ...
            $shipping = count($cart) > 0 ? 10.0 : 0; // Flat rate shipping
        ...
        @forelse ($cart as $key => $item)
            <div class="aq-cartmini-product-item mb-15 item-delete d-flex align-items-center">
                ...
                <div class="aq-cartmini-product-summary">
                    <h4 class="aq-product-title"><a
                            href="{{ route('products.show', $item['slug']) }}">{{ $item['name'] }}</a></h4>
                    @if(isset($item['size']))
                        <span class="aq-cartmini-product-size"><label>Size:</label> {{ $item['size'] }}</span>
                    @endif
                    <span class="aq-cartmini-product-price">${{ number_format($item['price'], 2) }}</span>
                    <div class="aq-product-details-quantity d-flex align-items-center">
                        <div class="aq-product-quantity">
                            ...
                            <input class="aq-cart-input" type="text" value="{{ $item['quantity'] }}">
                        ...
                        <form action="{{ route('cart.remove', $key) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="aq-line-anim aq-cartmini-remove aq-remove">Remove</button>
                        </form>
                    </div>
                </div>
            </div>
        @empty
            <div class="aq-cartmini-empty text-center p-4">
                <p>Your cart is empty.</p>
                <a href="{{ route('products.index') }}" class="aq-btn-black radius-30">Shop Now</a>
            </div>
        @endforelse
    </div>
    <div class="aq-cartmini-footer">
        <div class="aq-cartmini-total mb-25">
            <div class="aq-cartmini-total-item d-flex justify-content-between mb-5">
                <span>Sub Total</span>
                <span class="fs-bold">${{ number_format($subtotal, 2) }}</span>
            </div>
            <div class="aq-cartmini-total-item d-flex justify-content-between mb-5">
                <span>Shipping</span>
                <span class="fs-bold">${{ number_format($shipping, 2) }}</span>
            </div>
            <div class="aq-cartmini-total-item d-flex justify-content-between">
                <span>Total</span>
                <span class="aq-cartmini-total-price">${{ number_format($total, 2) }}</span>
            </div>
        </div>
        <div class="aq-cartmini-btn">
            <a href="{{ route('cart.index') }}" class="aq-btn-black w-100 radius-30 mb-15">View Cart</a>
            <a href="{{ route('order.checkout') }}" class="aq-btn-black btn-coffee-bg w-100 radius-30">Checkout</a>
        </div>
    </div>
</div>
